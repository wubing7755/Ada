# Phase Pass/Fail Review Example: Atlas MovePanel Display-State / Undo Semantics
...
- Phase: `Atlas Phase C: MovePanel display-state and undo semantics`
...
- Source/tests inspected:
  - `src/Atlas/Services/Commands/MovePanelCommand.cs`
  - `tests/Atlas.Tests/Services/Commands/MovePanelCommandTests.cs`
  - `src/Atlas/Services/DragService.cs`
  - `tests/Atlas.Tests/Services/DragServiceTests.cs`
...
```bash
dotnet test tests/Atlas.Tests/Atlas.Tests.csproj --filter 'FullyQualifiedName~MovePanelCommandTests|FullyQualifiedName~AtlasDragServiceTests' -v q

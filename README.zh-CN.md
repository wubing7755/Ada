...
人设与工程原则由 [SOUL.md](SOUL.md) 统一定义，61 个分发技能位于 [skills/](skills/)。
...
Ada 的 61 个技能遵循 Agent Skills 的 progressive disclosure：discovery 阶段只加载技能名称和 `description`；任务匹配后加载对应 `SKILL.md` 获得执行协议；需要时才读取 `references/`、`scripts/`、`assets/` 和 `evals/`。

因此用户通常只需描述目标，不需要记住或手工调用 61 个技能名称；Agent 会根据任务自动加载匹配的技能。
...
└── skills/software-development/  # 61 个 ada-* 技能
```

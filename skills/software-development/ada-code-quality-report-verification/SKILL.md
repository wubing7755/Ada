...
description: "Use when independently verifying claims in a code quality report — read actual source code, run reproducibility tools, and classify each issue as confirmed, partially correct, or false positive. For single-agent or sub-agent deployment."
version: 1.0.0
author: Hermes Agent
license: MIT
...
metadata:
  hermes:
    ...
    related_skills: [ada-code-quality-analysis, ada-code-efficiency-review, ada-requesting-code-review, ada-code-quality-pipeline]
---
...
Full pitfall catalog: `skill_view(name="ada-code-quality-report-verification", file_path="references/verification-pitfalls.md")` — covers methodological traps (trusting raw tools, skipping methodology, not verifying every claim), statistical traps (token-window bias, claim-by-claim verification), and quality traps (no reproducibility, accepting unverified claims).

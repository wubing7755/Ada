...
- 术语审计详细操作参见 SRS 审查方法论 Pass C（术语一致性审计：扫描→分类→术语表→批量替换→验证）
...
- **系统性术语审查**：当文档出现大量术语混用时，运行术语一致性审计流程（扫描→分类→术语表提案→批量替换→手动重写→双语注释→验证）。核心流程：扫描→分类→术语表提案→批量替换→手动重写→双语注释→验证。

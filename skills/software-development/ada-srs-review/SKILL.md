...
- **Stacking diagrams must show the Lib model correctly**: Only ONE panel is visible in a Dock region at a time; others collapse to LT/RT ToolBar entries — NOT stacked content rectangles. Include LT/RT sidebars with entry badges, before/after states with toolbar migration, and never show two panels with a separator line. Use "同区域其余面板收起为 LT/RT 条目", not "更多面板可堆叠". Use `viewBox="0 0 760 300"`.
